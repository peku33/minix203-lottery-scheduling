 ...
 *				pick_proc				     * 
 *===========================================================================*/
#include "/usr/src/lib/ansi/rand.c"
 ...
 */
  ...
  unsigned short selected_ticket;
  ...
  /*
	...
	Sprawdzamy czy w ogóle jest z czego losować, inaczej podzielimy przez 0
  */
  if(total_tickets > 0)
  {
	  /*
		Losujemy liczbę z zakresu <1; total_tickets> i na tej podstawie wybieramy proces.
		Za total_tickets odpowiada ready i unready
	  */
	  selected_ticket = (rand() % total_tickets) + 1;
	  rp = rdy_head[USER_Q];
	  while(selected_ticket > rp->num_tickets)
	  {
		  selected_ticket -= rp->num_tickets;
		  rp = rp->p_nextready;
	  }
 ...
 */
  ...
  /*
	Szukamy proces za którym wstawić nowy gotowy, aby zachować porządek nierosnący.
	Dzięki temu przeszukiwanie listy przez pick_proc będzie najszybsze
  */

  /* Na pewno dodajemy liczbę ticketów nowego procesu do liczby wszystkich ticketów */
  total_tickets += rp->num_tickets;

  /* Jeśli nie ma żadnego gotowego procesu - ten staje się pierwszym */
  ...
  /* Sprawdzamy, czy nie wpakować go przed pierwszy */
  if(rp->num_tickets > rdy_head[USER_Q]->num_tickets)
  {
	rp->p_nextready = rdy_head[USER_Q];
	rdy_head[USER_Q] = rp;
	return;
  }
  /* Szukamy miejsca do którego go wpakować */
  xp = rdy_head[USER_Q];
  while(xp->p_nextready && xp->p_nextready->num_tickets > rp->num_tickets)
	xp = xp->p_nextready;
  
  rp->p_nextready = xp->p_nextready;
  xp->p_nextready = rp;
}
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
	...
	qtail = &rdy_tail[SERVER_Q];
	  	...
	  	/* Od sumy odejmujemy priorytet tego */
		total_tickets -= rp->num_tickets;

	  	/* Jeśli proces który przestał być gotowy był głową - głową staje się następny proces. Zachowuje to kolejność i na pewno jest najszybszym rozwiązaniem */
		...
		{
			/* Szukamy poprzedniego, aby załatać dziurę w liście */
			xp = rdy_head[USER_Q];
			while(xp && xp->p_nextready != rp)
				xp = xp->p_nextready;

			xp->p_nextready = rp->p_nextready;
		}

		/* Jeśli to był chodzący proces - musimy wybrać inny */
		if(rp == proc_ptr)
			pick_proc();

		/* Nie potrzebujemy tego syfu na dole */
		return;
  }
